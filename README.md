...
- 첫 두 열의 한국어·영어 순서를 자동 판별하고 저장된 데이터셋 삭제
- 영어를 듣고 따라 말하기 / 한국어를 듣고 영어로 말하기
- Android TTS 예문 재생과 음성 인식 기반 발음 유사도 채점
- 연결된 블루투스 헤드셋 마이크 우선 사용 및 현재 입력 장치 표시
- 긴 문장 인식 대기시간·후보·문장별 단어 힌트 보강 및 실제 인식 문장 표시
- 말하는 동안 인식된 단어를 실시간 파란색으로 표시
- 발음 후 다시 발음하거나 다음 문장으로 직접 이동
- 인식 문장 별도 표시를 없애고, 선택 버튼을 카드 하단에 항상 고정
- 설정 저장 버튼 고정, TTS 초기화 대기 및 더 빠른 부분 음성 인식
- 문장별 반복 횟수 1~5회 선택 및 저장
- 반복 중 통과 시 다음 반복으로 자동 이동, 마지막 반복 뒤 자동/수동 이동 선택
- 한국어 듣기 모드의 일부 단어 가림 영어 힌트와 발음 후 점수 표시
...
첫 번째와 두 번째 열에 한국어와 영어를 넣으면 순서를 자동으로 판별합니다. 첫 번째 시트만 읽으며, 제목 행과 빈 셀 쌍은 건너뜁니다. C열 이후의 데이터는 자동으로 무시합니다.
